...
# (antes apuntaba a db/sirgas_tropo.db, que nunca se creó -- la ingesta real
# escribe en data/andes_observa.db, en la raíz del proyecto)
...
async def get_velocidades():
    """Endpoint para obtener velocidades de estaciones (simulado por ahora)"""
    return {
        "estaciones": [
...
async def get_estaciones_tropo():
    ...
    Tabla real: tropo_observations (columna station_id) relacionada
    con stations (columna code) -- requiere JOIN, no es una sola tabla plana.
    """
...
async def get_serie_tropo(
    ...
    para una estación específica. Columnas reales: ztd_total_mm y
    ztd_stddev_mm (no 'ztd'/'ztd_error' como en la versión anterior).
    """
    conn = get_db_connection()
    if not conn:
